...
export const BidProvider = ({ children }) => {
    ...
    const fetchUserBidSummary = useCallback(async (currentUserId) => {
        ...
        setLoadingBids(true);
        try {
            const response = await fetch(`https://a10-freelance-marketplace-server.vercel.app/bids/user/${currentUserId}/summary`);
            if (!response.ok) {
    ...
    const addBid = useCallback(async (taskId) => { 
        ...
        try {
            const response = await fetch('https://a10-freelance-marketplace-server.vercel.app/bids', {
                method: 'POST',
